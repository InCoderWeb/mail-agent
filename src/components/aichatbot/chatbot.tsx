...
export default function Home() {
    ...
    async function startAI() { // Function to start the AI agent (Starts when clicked on the button)
        ...
        else { // If Error while starting the AI agent
            setChatHistory([{
                role: "assistant",
    ...
    async function sendMessage() { // Function to send a message to the AI agent
        setChatHistory(prevChat => [...prevChat, { role: "user", content: message }]); // Add the user's message to the chat history
        ...
        console.log(res); // To check the response from the server
        if (res.status == 200) {
            setChatHistory(prevChat => [...prevChat, { role: "assistant", content: res.reply }]); // Add the assistant's reply to the chat history
        }
        else { // If Error while sending the message to the AI agent
            setChatHistory(prevChat => [...prevChat, { role: "assistant", content: "AI is not ready to assist you!" }]);
        }
    }
    ...
    return (
        ...
        {/* Button to open the chat window and start the AI agent */}
            ...
            { /* If the chat window is open, display the chat container */
                isOpen && (
                    ...
                    >
                        <div className="flex flex-col space-y-1.5 pb-6"> {/* Header of the chat window */}
                        ...
                        <div 
                            className="pr-4 h-[474px] overflow-y-scroll flex flex-col gap-5 mb-8"
                            style={{ minWidth: "100%" }}
                            ...
                            { /* Map through the chat history and display each message */
                                chatHistory.map((message, index) => {
                                    if (message.role == 'user') { // If the message is from the user
                                        // Display the user's message with a different style
                                        return (
                                            <div key={index} className="flex gap-3 text-gray-600 text-sm">
                                                ...
                                                <p className="leading-relaxed">
                                                    <span className="block font-bold text-gray-700">You </span>{message.content}
                                                </p>
                                            </div>
                                        )
                                    }
                                    else {
                                        return ( // If the message is from the assistant
                                            // Display the assistant's message with a different style
                                            <div key={index} className="flex gap-3 text-gray-600 text-sm">
                                                <span className="relative flex shrink-0 overflow-hidden rounded-full w-8 h-8">
                                                    <div className="rounded-full bg-gray-100 border p-1">
                                                        <svg
                                                ...
                                                <p className="leading-relaxed">
                                                    <span className="block font-bold text-gray-700">Assistant </span>{message.content}
                                                </p>
                                            </div>
                                        )
                                    }
                        ...
                        <div className="flex items-center pt-0"> {/* Input field for sending messages */}
